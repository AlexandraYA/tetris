...
Tetris.prototype.draw = function(preview : number, coords : any) {
  ...
  for (let x in this.currentElem) {
      ...
      for (let j : number = 0; j < this.currentElem[x].length; j++) {
          ...
          if (tempX < 0) {
              ...
              this.bottomFieldEdge[tempX].forEach(function(fieldY : number) {
...
Tetris.prototype.rowRemove = function(yFullRowInArray : string) {
  ...
  for (let i : number = 0; i < this.fieldElemLinks.length; i++) {
      if (parseInt(this.fieldElemLinks[i].style.top) == yFullFieldRow) {
...
Tetris.prototype.controlKeyDown = function(e: Event) {
  e.stopPropagation();
  e.preventDefault();
